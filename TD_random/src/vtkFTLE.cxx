                                ...
                                vtkInformationVector **inputVector, vtkInformationVector *outputVector)
{
    ...
    outInfo1->Set(vtkStreamingDemandDrivenPipeline::UPDATE_EXTENT(), extentSeedGrid,6);
    outInfo1->Set(vtkStreamingDemandDrivenPipeline::WHOLE_EXTENT(), extentSeedGrid,6);
                         ...
                         vtkInformationVector *outputVector)
{
    ...
    double* times = outputVector->GetInformationObject(0)->Get(vtkStreamingDemandDrivenPipeline::TIME_STEPS());
    int numInTimes = outputVector->GetInformationObject(0)->Length(vtkStreamingDemandDrivenPipeline::TIME_STEPS());
    ...
    {
        for ( unsigned int k = 0; k < numInTimes - 1; k++)
        {
        ...
        for ( unsigned int l = 0; l < numInTimes - 1; l++)
        {
    ...
    std::cout<< "Integer Range of time steps" << InitialTime <<" and " << FinalTime<<std::endl;


    std::vector<vector<double>> previousPts;

    //for (unsigned int i = 0; i < numTimeSteps - 1; i ++)
    for (unsigned int i = InitialTime; i < FinalTime;  i ++)
    {
    ...
    int extentSeedGrid[6];
       ...
       // this->nextseedGrid->SetSpacing(this->spacingSeedGrid);
