...
public:
    ...
    Integrator(int intDirection, double stepsize, double starttime, double endtime, double stagThresh,
               vtkSmartPointer<vtkImageData> inputGrid, vtkSmartPointer<vtkImageData> source,
               vtkSmartPointer<vtkImageData> nextinputGrid, unsigned int blockNum, unsigned int initialnum, unsigned int finalnum, double* time, std::vector<vector<double>> previousPts);
    ~Integrator();
