                         ...
                         vtkInformationVector *outputVector)
{
    ...
    auto InitialTime = static_cast<unsigned int>(this->tauInitial);

    unsigned int FinalTime;

    if ( this->tauFinal >= numTimeSteps )
    {
        std::cerr << "Incorrect final time values" << std::endl;
        return false;
       ...
       // this->nextseedGrid->SetSpacing(this->spacingSeedGrid);
        ...
        if (this->useGPU) {
            ...
            Integrator *customIntegrator = new Integrator(temp, this->integrationStepSize, fabs(this->tauInitial), fabs(this->tauFinal), LIMIT_DOUBLE,
                                                          this->inputGrid, this->seedGrid, this->nextinputGrid, i, previousPts);
