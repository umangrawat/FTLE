...
int vtkFTLE::RequestDataObject( vtkInformation*, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
    ...
    if (input)
    {
        ...
        for(int i=0; i < this->GetNumberOfOutputPorts(); ++i)
        {
            ...
            //vtkMultiBlockDataSet *output = vtkMultiBlockDataSet::SafeDownCast(info->Get(vtkDataObject::DATA_OBJECT()));       //// commented out
                         ...
                         vtkInformationVector *outputVector)
{
    ...
    //vtkMultiBlockDataSet *output = vtkMultiBlockDataSet::SafeDownCast(outInfo->Get(vtkDataObject::DATA_OBJECT()));    //// commented out these lines
    //vtkMultiBlockDataSet *output1 = vtkMultiBlockDataSet::SafeDownCast(outInfo1->Get(vtkDataObject::DATA_OBJECT()));

    //vtkSmartPointer<vtkMultiBlockDataSet> newpoints = vtkSmartPointer<vtkMultiBlockDataSet>::New();

    //vtkSmartPointer<vtkPolyData> addoutputData = vtkSmartPointer<vtkPolyData>::New();
    ...
    */
        ...
        flowMap->SetNumberOfComponents(2);                                                              ///changed to 2
    ...
    if ( this->tauFinal + this->tauInitial > times[numInTimes - 1] || this->tauInitial < times[0])                       ////added this line
    {
        ...
        if (this->seedGrid->GetNumberOfPoints() == previousPts.size()) {      ////
            ...
            for (int indexLine = 0; indexLine < previousPts.size(); ++indexLine) {        ////
                ...
                pathlineEndpoint = previousPts.at(indexLine);
                    //flowMap->SetTuple(indexLine, pathlineEndpoint[0], pathlineEndpoint[1]);
                    flowMap->SetTuple(indexLine, pathlineEndpoint);
                    seed[0] = 1;
                    seeded->SetTuple(indexLine, seed);
                } else {
                    std::cerr << "stream Line not seeded" << std::endl;
    ...
    /*
        ...
        SeedgridappendImage->AddInputData(this->seedGrid);
        SeedgridappendImage->Update();

        newlines->ShallowCopy(StreamLinesappendFilter->GetOutput());
*/
        /*
        ...
        */
        ...
        //output1->ShallowCopy(appendFilter->GetOutput());


       // }

    output->ShallowCopy(this->seedGrid);
    output1->ShallowCopy(pathLines);
...
vtkSmartPointer<vtkDoubleArray> vtkFTLE::computeDerivatives(vtkSmartPointer<vtkImageData> data, char* arrayName)
{
    ...
    for (int z=0; z<1; z++)                                     ///// changed to 1
    {
        for (int y=0; y<dataDims[1]; y++)
        {
            for (int x=0; x<dataDims[0]; x++)
