...
bool Integrator::cellLocator(vec2& output, vec2& nextoutput, vec2 location)
{
   ...
   // loc[2] -= origin[2];
    ...
    if(xIndex > dim[0]-2 || yIndex > dim[1]-2                                                   ///why dim[0] - 2, check for dim[0] - 1
       || xIndex < 0 || yIndex < 0)
    {
        return false;
    }
...
vtkSmartPointer<vtkPoints> Integrator::integratePointRK4(vec2 startLocation)
{
    ...
    while ( currentIntTime < timeDiff && currentIntLength < this->integrationTime )                    ///remove equality sign
    //while (currentIntLength <= this->integrationTime)
    {
        pointValid = true;
        vec2copy(currentLocation, currentLocationSave);
        ...
        vec2copy(velCurrLocation, vel);
        vec2copy(velNextLocation, nextvel);
        vec2lint(vel, nextvel, intFac, temp);                                                               ///// interpolation ain't right, need to normalize the interpolation factor
        vec2copy(temp, vel);
        ...
        if (this->iter == this->initialNum && currentIntTime == this->startTime - this->timestep[this->iter] )
        {
            ...
            //auto nearestValue = static_cast<double>(ceilf(static_cast<float>(this->startTime) * 10)/10);
            double nearestValue = floor(this->startTime * pow ( 10.0, count - 1 )) / pow ( 10.0, count - 1 ) + stepSize;
            double step = nearestValue - this->startTime;

            //std::cout << nearestValue <<std::endl;
        ...
        else if ( this->integrationTime - currentIntLength < stepSize )
        {
            double interpolator = this->integrationTime - currentIntTime;    //// new step size
            ...
            //currentIntTime += stepSize;
            //currentIntLength += stepSize;
            ////let's store the exact points conditoons
            currentIntTime += interpolator;
            currentIntLength += interpolator;
        ...
        //length += vec2mag(currentLocation);

        //std::cout <<"current int time " << currentIntTime << " cur int len " << currentIntLength << " time diff " << timeDiff << " end time " << this->integrationTime << std::endl;

        //if (currentIntTime > timeDiff)                              ////save te endpoints for the next iteration
        if (currentIntTime == timeDiff || currentIntLength == this->integrationTime)                                          /// do not need second condition and check for equals to and changed to equality
        //if ( currentIntLength > this->integrationTime)
        {
            std::vector<double> currloc = {currentLocation[0], currentLocation[1]};
            this->PathlineEndPoints.push_back(currloc);
            //std::cout << "writing pathline pts" << std::endl;
        }
