                        ...
                        int blockNum,  int initialnum,  int finalnum, double* time, std::vector<vector<double>> previousPts)
{
    ...
    this->timestep = time;
    //this->integrationTime = time[blockNum + 1] - time[blockNum];
...
vtkSmartPointer<vtkPoints> Integrator::integratePointRK4(vec2 startLocation)
{
    ...
    vtkSmartPointer<vtkPoints> outputPoints = vtkSmartPointer<vtkPoints>::New();
    double currentIntTime = 0;
    double timeDiff = this->timestep[this->iter + 1] - this->timestep[this->iter];    /// Time gap bwtween 2 consecutive intervals
    double currentIntLength = timestep[this->iter];
    double length = 0.;
    ...
    while (currentIntTime <= timeDiff && currentIntLength <= this->endTime)
    {
        pointValid = true;
            ////changed 3 to 2
        vec2copy(currentLocation, currentLocationSave);
        ...
        if (currentIntTime >= timeDiff || currentIntLength >= this->endTime)
        {
            std::vector<double> currloc = {currentLocation[0], currentLocation[1]};
            this->PathlineEndPoints.push_back(currloc);
