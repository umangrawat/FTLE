...
vtkSmartPointer<vtkPoints> Integrator::integratePointRK4(vec2 startLocation)
{
    ...
    //{
        while (currentIntTime <= timeDiff && currentIntLength <= this->endTime)
        {
            ...
            {
                auto nearestValue = static_cast<double>(ceilf(static_cast<float>(currentIntTime) * 1000)/1000);
                double step = nearestValue - currentIntTime;

                //std::cout <<"nearest " << nearestValue <<std::endl;
                if (fabs(step) < LIMIT_DOUBLE)
                {
                    step = stepSize;
                }

                std::cout << "step size at start" << step <<std::endl;
