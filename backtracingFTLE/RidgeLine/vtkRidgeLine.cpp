...
int vtkRidgeLine::RequestData(vtkInformation *vtkNotUsed(request), vtkInformationVector **inputVector, vtkInformationVector *outputVector) {
    ...
    for (int len  = 0; len < ridgeLinelength.size(); len++)
    {
        ...
        RidgeLineLengthArray->InsertTuple1(idLen, length);
    ...
    vtkSmartPointer<vtkPassArrays> RidgelineLength = vtkSmartPointer<vtkPassArrays>::New();
    RidgelineLength->SetInputData(linesPolyData);
    RidgelineLength->AddArray(vtkDataObject::FIELD, "RidgeLineLengthArray");

    RidgelineLength->Update();

    vtkSmartPointer<vtkPolyData> ridgelineslen = vtkPolyData::SafeDownCast(RidgelineLength->GetOutput());;
