...
void DataDict::setDataDict(char* dirName, int size, bool verb, bool mmap)
{
  ...
  while (true) {
    ...
    float time;
    if (sscanf(fileName, "%f", &time) != 1) continue;
  ...
  for (int i = 0; i < cacheSize; i++) {         // allocate
    if (useMMap) {
      cache[i] = NULL;
      fileDescs[i] = -1;
    }
...
TimeStep* DataDict::getTimeStep(int timeStepNr)
{
  ...
  if (!ts->data) {                              // need to cache it
    int oldTimeStepNr = cached[cachePos];
    cached[cachePos] = timeStepNr;
    if (oldTimeStepNr >= 0) timeStep[oldTimeStepNr].data = 0;

    if (useMMap) {
      // unmap and close old file
      if (verbose) fprintf(stderr, "unmapping and closing\n");
      if (oldTimeStepNr >= 0) {
      ...
      if (verbose) fprintf(stderr, "opening\n");
      ...
      // map new file
      if (verbose) fprintf(stderr, "Cache mmap: %s (%d bytes)\n", ts->fileName, nBytes);
      ...
      if (verbose) fprintf(stderr, "Cache load: %s (%d bytes)\n", ts->fileName, nBytes);
...
{
  if (t < minTime() + epsilon) {                // first time step
    ts0 = getTimeStep(0);
    ts1 = getTimeStep(1);
    wt0 = 1;
    wt1 = 0;
  ...
  else if (t > maxTime() - epsilon) {           // last time step
  ...
  else {                                        // binary search for time
    int lo = 0;
    int hi = nTimeSteps-1;
    bool found = false;
    while (hi > lo+1) {
      int mid = (lo + hi) / 2;
      if (t < timeStep[mid].time - epsilon) hi = mid;
      else if (t > timeStep[mid].time + epsilon) lo = mid;
      else {                                // intermediate time step
        ts0 = getTimeStep(mid);
...
#ifdef OLD
      if (t > time) { ts0 = getTimeStep(lo); ts1 = getTimeStep(hi); }
      else          { ts1 = getTimeStep(hi); ts0 = getTimeStep(lo); }
#endif
      ts0 = getTimeStep(lo); 
	  ts1 = getTimeStep(hi);
