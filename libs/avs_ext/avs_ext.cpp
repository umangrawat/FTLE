...
{ // returns index of dest_idx-th component < dest_veclen (if compare < 0)
  ...
  int idx = 0;
  for (int c=0; c<cNb; c++) {
    if ((compare < 0) && (ucd->node_components[c] < dest_veclen) ||
        (compare == 0) && (ucd->node_components[c] == dest_veclen) ||
        (compare > 0) && (ucd->node_components[c] > dest_veclen)) {
      if (idx == dest_idx) {
        return c;
      }
...
{ // returns zero on success
  ...
  for (int c=0; c<compNb; c++) {
    ...
    if (c == comp_idx) {
      int cpylen;
      if (c != compNb - 1) cpylen = ptr - lastptr;
      else cpylen = strlen(lastptr);
...
{ // updates AVS choice and determines selected UCD component
  ...
  if (!(ucd->node_labels && strlen(ucd->node_labels) > 0)) return -1;
