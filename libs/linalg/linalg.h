...
////new insertion
inline void mat2setrows(mat2 a, vec2 a0, vec2 a1)
{
...
inline void vec2lint(vec2 v0, vec2 v1, double t, vec2 v)
// Do a linear interpolation given the 2 corner values and the variable
{
    vec2 tmp;

    vec2zero(v);
    vec2scal(v0, (1-t), tmp);  vec2add(v, tmp, v);
    vec2scal(v1,   t  , tmp);  vec2add(v, tmp, v);
}

inline void vec2bilint(vec2 v00, vec2 v10, vec2 v01, vec2 v11, double s, double t, vec2 v)
...
// given the 4 corner values and the two parameters
{
    ...
    vec2zero(v);
    vec2scal(v00, (1-s)*(1-t), tmp);  vec2add(v, tmp, v);
    vec2scal(v10,   s  *(1-t), tmp);  vec2add(v, tmp, v);
    vec2scal(v01, (1-s)*  t  , tmp);  vec2add(v, tmp, v);
    vec2scal(v11,   s  *  t  , tmp);  vec2add(v, tmp, v);
}
