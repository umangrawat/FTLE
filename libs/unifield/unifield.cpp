...
{ // compNames: may be NULL and may have NULL entries
  ...
  default: printf("UniField: ERROR: unsupported data type\n");
  }
...
#  else
  ...
  if (!vtkFld) vtkFld = vtkStructuredGrid::New();
  ...
  for (int c=0; c<nComp; c++) {
    ...
    if (compNames && compNames[c]) dat->SetName(compNames[c]);
    else {
      char buf[256];
      sprintf(buf, "%p", dat);
      dat->SetName(buf);
...
#  endif
{
  if (covGridP) *covGridP = covGrid;
  if (covScalarDataP) *covScalarDataP = covScalarData;
  if (covVector2DataP) *covVector2DataP = covVector2Data;
} 
#endif
...
#  endif
{
  if (covGridP) *covGridP = covGrid;
  if (covScalarDataP) *covScalarDataP = covScalarData;
  if (covVector3DataP) *covVector3DataP = covVector3Data;
} 
#endif
