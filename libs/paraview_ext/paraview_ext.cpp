...
vtkFloatArray* castToVtkFloatArray(vtkDataArray* inArray) {
  ...
  if (!inArray) {
    /// if inArray is null pointer, return null pointer as well
    
    return NULL;
  }
  vtkSmartPointer<vtkFloatArray> inArrayFloat 
    = vtkFloatArray::SafeDownCast(inArray);
  if (!inArrayFloat) {

    inArrayFloat = vtkFloatArray::New();
    inArrayFloat->SetName(inArray->GetName());
    const int componentCount = inArray->GetNumberOfComponents();
    const int tupleCount = inArray->GetNumberOfTuples();
    inArrayFloat->SetNumberOfComponents(componentCount);
    for (vtkIdType iTuple = 0; iTuple < tupleCount; iTuple++) {
      
      /// ugly, but component count must be known at compile time...
      if (componentCount == 1) {
        
        const vtkTuple<float, 1> t = vtkTuple<double, 1>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 2) {
        
    	const vtkTuple<float, 2> t = vtkTuple<double, 2>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 3) {
        
    	const vtkTuple<float, 3> t = vtkTuple<double, 3>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 4) {
        
        const vtkTuple<float, 4> t = vtkTuple<double, 4>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 5) {
        
        const vtkTuple<float, 5> t = vtkTuple<double, 5>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 6) {
        
        const vtkTuple<float, 6> t = vtkTuple<double, 6>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 7) {

	    const vtkTuple<float, 7> t = vtkTuple<double, 7>(
              inArray->GetTuple(iTuple)).Cast<float>();
	    inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 8) {

        const vtkTuple<float, 8> t = vtkTuple<double, 8>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else if (componentCount == 9) {

        const vtkTuple<float, 9> t = vtkTuple<double, 9>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextTuple(t.GetData());
      } else {
        
        printf("Unconsidered component count in ");
        printf("lib/paraview_ext/paraview_ext.cpp, ");
        printf("component count was %d. ", componentCount);
        printf("Treating as one component.\n");
        const vtkTuple<float, 1> t = vtkTuple<double, 1>(
          inArray->GetTuple(iTuple)).Cast<float>();
        inArrayFloat->InsertNextValue(*t.GetData());
      }
    }
  }
...
{  
  ...
  for (int c=0; c<unif->getCompNb(); c++) {
    ...
    if (unif->getCompName(c)) dat->SetName(unif->getCompName(c));
    else {
      char buf[256];
      sprintf(buf, "%p", dat);
      dat->SetName(buf);
...
{ // pass data (interpolate from input)
  ...
  Unstructured *unst_all;
  {
    ...
    for (int a=0; a<input->GetPointData()->GetNumberOfArrays(); a++) {
      ...
      vtkFloatArray* inArrayFloat = castToVtkFloatArray(inArray);
      if (inArray->GetNumberOfComponents() == 1) {
        svec.push_back(inArrayFloat);
      } else {
        vvec.push_back(inArrayFloat);
      }
    }
