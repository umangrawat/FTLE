...
class coIntScalarParamExt : myParam
{
 ...
 public:
  ...
  coIntScalarParamExt() { myParams.push_back(this); }
  ~coIntScalarParamExt()
  {
  ...
  void setValue(int v) { p->setValue(v); }
  int getValue(void) { return p->getValue(); }

  void enable(void) { p->enable(); }
  void disable(void) { p->disable(); }

  void show(void) { p->show(); }
  void hide(void) { p->hide(); }
  ...
  {
    int val = p->getValue();
    if (val < min) p->setValue(min);
    if (val > max) p->setValue(max); 
  }
};
...
class coFloatParamExt : myParam
{
 ...
 public:
  ...
  coFloatParamExt() { myParams.push_back(this); }
  ~coFloatParamExt()
  {
    for (int i=0; i<(int)myParams.size(); i++) {
      if ((myParams.size() > 0) && (myParams[i] == this)) {
  ...
  void setValue(float v) { p->setValue(v); }
  float getValue(void) { return p->getValue(); }
